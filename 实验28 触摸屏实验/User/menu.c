...
#define C_BTN_VID   0xFD20u    /* orange  */
...
/* -----------------------------------------------------------------------
 * Layout  (portrait 240x320) -- 4 buttons
 * Title   y=18
 * Sub     y=46
 * Btn 1   y=65..114   (50 px tall)
 * Btn 2   y=120..169
 * Btn 3   y=175..224
 * Btn 4   y=230..279
 * Hint    y=300
...
/* "VIDEO" ASCII only - font table missing 视播放, use ASCII label */
static const char STR_VID[]         = "VIDEO";
...
static void draw_menu(void)
{
    ...
    /* BTN4: orange - VIDEO (ASCII, 5 chars * 8px = 40px) */
    lcd_draw_rectangle((uint16_t)(BTN_X - 1u), BTN4_Y - 1u,
                       (uint16_t)(BTN_X + BTN_W), (uint16_t)(BTN4_Y + BTN_H), C_BORDER);
    lcd_fill(BTN_X, BTN4_Y, (uint16_t)(BTN_X + BTN_W - 1u),
             (uint16_t)(BTN4_Y + BTN_H - 1u), C_BTN_VID);
    lcd_show_string((uint16_t)(BTN_X + (BTN_W - 40u) / 2u),
                    (uint16_t)(BTN4_Y + (BTN_H - 16u) / 2u),
                    40u, 16u, 16u, (char *)STR_VID, C_BTN_TEXT);
...
void main_menu_run(void)
{
    ...
    while (1)
    {
        ...
        if (down && !prev_down)   /* leading-edge touch only */
        {
            ...
            else if (hit_test(tx, ty, BTN_X, BTN4_Y, BTN_W, BTN_H))
            {
                /* ASCII button flash */
                lcd_fill(BTN_X, BTN4_Y, (uint16_t)(BTN_X + BTN_W - 1u),
                         (uint16_t)(BTN4_Y + BTN_H - 1u), C_PRESS);
                lcd_show_string((uint16_t)(BTN_X + (BTN_W - 40u) / 2u),
                                (uint16_t)(BTN4_Y + (BTN_H - 16u) / 2u),
                                40u, 16u, 16u, (char *)STR_VID, C_BTN_TEXT);
                delay_ms(120u);
                video_player_run(); /* returns on EXIT touch */
                draw_menu();
            }
        }
